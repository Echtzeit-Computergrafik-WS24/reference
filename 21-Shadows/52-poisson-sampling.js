...
const geoFSSource = `#version 300 es
    ...
    float calculateShadow() {
        ...
        float bias = max(0.006 * (1.0 - dot(f_normal, u_lightDirection)), 0.001);
        ...
        for (int i=0; i<16; i++){
            int index = int(16.0*random(floor(f_posWorldSpace.xyz * 1000.0), i))%16;
            visibility += texture(u_texShadow, vec3(projCoords.xy + poissonDisk[index] * 0.002, projCoords.z - bias));
        }
