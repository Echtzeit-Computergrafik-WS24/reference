...
const geoFSSource = `#version 300 es
    ...
    vec2 parallax_mapping(vec3 viewDir) {
        ...
        float depth = 1.0 - texture(u_texDepth, f_texCoord).r;
        vec2 texOffset = (viewDir.xy / viewDir.z) * (depth * parallaxScale);
        return f_texCoord + texOffset;
    }
...
const geo = glance.createDrawCall(gl, "geo",
    ...
    geoProgram,
    {
        textures: {
            ...
            u_texDepth: await glance.loadTexture(gl, "/img/pebbles-depth.webp"),
        },
        depthTest: gl.LESS,
        cullFace: gl.BACK,
    }
