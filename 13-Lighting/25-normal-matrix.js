...
const vertexShaderSource = `#version 300 es
    ...
    out float f_lightIntensity;

    void main() {
        vec4 worldPos = u_modelXform * vec4(a_pos, 1.0);
        vec4 normal = u_modelXform * vec4(a_normal, 0);
        f_lightIntensity = max(0.0, dot(u_lightDirection, normal.xyz));
...
const fragmentShaderSource = `#version 300 es
    ...
    in float f_lightIntensity;

    out vec4 o_fragColor;

    void main() {
        vec3 color = vec3(1.0); // white
        color *= f_lightIntensity;
...
function myRenderLoop({ globalTime })
{
    ...
    quad.uniform.u_modelXform = glance.Mat4.rotateX(globalTime / 3000).rotateY(globalTime / 2000);
    glance.draw(gl, quad);
}
